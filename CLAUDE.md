...
- `/` — участник: регистрация → вкладки «Список / (код) / Карта».
...
`frontend/src/map/building.svg` — изометрическая перевёрнутая «П» (viewBox 1000×720), снизу по центру снаружи «Вход». Редактируется руками; метки станций накладываются поверх в долях от размера, поэтому правка SVG не ломает координаты (если не менять viewBox).
